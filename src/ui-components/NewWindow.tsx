...
class NewWindow extends React.PureComponent<Partial<NewWindowProps>, NewWindowState> {
  /**
   ...
   */
  openChild() {
    const { url, title, name, width, height, onBlock, onOpen, center } = this.props
        ...
        Array.from(styleSheet.cssRules).forEach((cssRule: CSSRule) => {
          ...
          if ([3, 5].includes(type)) {
            returnText = cssText
              ...
              .map(line => {
                if (line[1] === '/') {
                  return `${line.slice(0, 1)}${window.location.origin
                    }${line.slice(1)}`
                }
                return line
              })
